...
- **Worker private-beta write protection** uses `WORKER_API_TOKEN`. When configured, `/watchlist` and `/devices` require `Authorization: Bearer <token>`; the iOS client reads the same value from its launch environment. This is temporary and must be replaced with real user authentication before public release.
